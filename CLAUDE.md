...
| English `.en.md` (translation, or hand-written half of a self-authored pair) | `<slug>.en.md` | **repo** (AI translation) or **vault** (hand-written, synced) | omit (locale from suffix) | AI → yes, human → **no** | yes (`sourceHash`); N/A for an independent original |

So the `.en.md` suffix means "the English companion at the canonical slug `<slug>`" — usually a translation of `<slug>.md`, but also the **hand-written English half of a bilingual pair you author yourself** (English first, Chinese `<slug>.md` to follow). An English-only **original** with no Chinese companion is instead a normal `<slug>.md` carrying `lang: en` (synced normally, ignored by the drift checker which only scans `*.en.md`). A **human** companion omits the AI-translated disclaimer (it's not machine-made). **Crucial pairing rule:** a `lang: en` `.md` can *never* pair with a Chinese companion — `getAlternateLocalePost` returns `undefined` for it — so to get the language toggle + reciprocal `hreflang`, the English must be the **`.en.md`** with Chinese as the canonical **`<slug>.md`** (not a `lang: en` `.md`). A hand-written `.en.md` may be authored in the vault; sync pulls it through (see *Survives sync* below), while AI translations stay repo-only. Key pieces:
...
- **Survives sync (two passes):** `scripts/sync.sh`'s `--delete` mirror **excludes** `*.en.md`, so repo-only AI translations are never wiped. A **second, no-delete** `rsync` pass then pulls any `.en.md` that *does* live in the vault (a hand-written English half of a self-authored pair) into the repo — so vault-authored English syncs automatically, while repo-only `.en.md` (absent from the vault) stay untouched. Same-folder placement means image paths (`assets/<slug>/…`) and the `![alt|600](…)` width syntax resolve identically to the Chinese post — no path rewrites. **Caveat:** while the Chinese `<slug>.md` is still `draft: true`, don't commit it (public repo → WIP leak); keep only the `.en.md` tracked. The English `.en.md` then ships solo (no toggle/`hreflang`, P3-clean) until the Chinese goes `draft: false` and is committed — at which point the reciprocal wiring appears automatically on the next build.
...
- **P5 — robots.txt** is already correct (the `astro-robots-txt` integration emits `Sitemap:` automatically, allow-all — AI crawlers intentionally not blocked); **404** is a single bilingual page with nav links (GitHub Pages can't serve a per-locale 404).
- **`/llms.txt`** ([llmstxt.org](https://llmstxt.org/)) is a **dynamic endpoint** — `src/pages/llms.txt.ts` — generated from the default-locale (zh) posts via `getAllPosts()`, so it stays in sync on every build (drafts filtered in PROD). Emits H1 title → blockquote description → author/lang note → `## Posts` (newest-first, absolute canonical zh URLs, `[title](url): description`) → `## Optional` (About, RSS). Uses **absolute URLs** (`new URL(path, import.meta.env.SITE)`) since an LLM reading it has no page base to resolve against. Lists only canonical zh URLs (each page carries its own hreflang to `/en/`); robots.txt does not (and need not) reference it — discovery is via the conventional `/llms.txt` path.
- **P5 content sweep done.** The CUDA series' absolute `xavierforge.dev` cross-links are now root-relative (zh in the vault, en in the `.en.md`). One image in `numba-cuda-puzzles-1` (`file-20260529134830454.png`) has an empty `![]()` alt **on purpose** — it's decorative, so `alt=""` is the correct semantic. Don't "fix" it in future alt audits.
